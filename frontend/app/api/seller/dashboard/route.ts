  ...
  try {
    ...
    for (const order of orders) {
      ...
      for (const item of order.products) {
        if ((item as any).sellerId?.toString() === sellerId) {
      ...
      if (hasSellerItems) {
        ...
        if ((order as any).status === 'Processing' || (order as any).status === 'Pending') {
          pendingOrders++;
        }
        ...
        if (recentOrders.length < 5) {
          recentOrders.push({
            id: order._id,
            customer: (order as any).shippingAddress?.name || 'Anonymous',
            amount: orderAmountForSeller,
            status: (order as any).status || (order.isDelivered ? 'Delivered' : (order.isPaid ? 'Processing' : 'Pending')),
